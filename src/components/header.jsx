    ...
    <>
      <header
        className={`sticky top-0 z-50 transition-all duration-500 ${
          scrolled ? "glass shadow-sm shadow-rose/10 py-2" : "bg-cream/90 backdrop-blur-md py-3"
        }`}>
        <div className="container-main flex items-center justify-between h-16 lg:h-20 px-4 sm:px-6 lg:px-8">
          {/* Mobile menu button */}
          <button
            className="lg:hidden p-2 text-accent hover:text-rose-dark transition-colors"
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block">
              <span className="font-display text-xl lg:text-2xl font-semibold text-accent tracking-wide">
                Lumière
              </span>
              ...
              <Link
                ...
                className={`text-sm uppercase tracking-[0.15em] font-medium transition-colors duration-300 relative after:absolute after:bottom-[-4px] after:left-0 after:h-[1.5px] after:bg-rose-dark after:transition-all after:duration-300 ${
                  location.pathname === link.to
                    ...
                    : "text-secondary hover:text-rose-dark after:w-0 hover:after:w-full"
                }`}>
                {link.label}
              </Link>
            ))}
          ...
          <div className="absolute top-0 left-0 h-full w-[min(320px,85vw)] bg-cream shadow-2xl animate-fade-up flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-blush">
              <Link to="/" className="flex items-center gap-3" onClick={() => setIsOpen(false)}>
                <img src={LOGO} alt="Lumière Beauty" className="w-10 h-10 rounded-full object-cover" />
                <span className="font-display text-xl font-semibold text-accent">Lumière</span>
              </Link>
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`py-4 px-4 rounded-xl text-lg font-medium transition-all duration-200 ${
                    location.pathname === link.to
                      ? "bg-rose-light text-accent"
                      : "text-secondary hover:bg-rose-light/50"
                  }`}>
                  {link.label}
                </Link>
              ))}
